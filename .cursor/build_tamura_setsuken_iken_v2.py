...
def build_body(orig_body):
    ...
    paras.append(mk_p(
        "aff1",
        "しかし、被疑事実の否認ないし黙秘という正当な防御権の行使それ自体を不"
        "利益に評価することは許されない。否認・黙秘を接見禁止の根拠として用いること"
        "は、自白を強要する違法捜査に手を貸す結果を招来するものであって到底許されな"
        "い。"
    ))
    paras.append(mk_p(
        "a1",
    ...
    paras.append(mk_p(
        "aff1",
        "しかし、最も近しい家族である内縁の妻について「属性が分からない」「暴"
        "力団との関係性が不明である」というのみで接見及び手紙の授受を一切認めないと"
        "いう結論は、あまりに乱暴である。大宮氏は本件事件とは一切無関係であり、共犯"
        "者とされている者たちとは全員夫を介して会ったことがあるに過ぎず、連絡先を交"
        "換しているわけではなく、田村氏を介さずにこれらの者と連絡を取る手段を持たな"
        "い。大宮氏は、弁護人から「罪証隠滅行為に加担すれば、あなたが犯罪に問われる"
        "」と説明を受け、これらの人と連絡を取ることは証拠の隠滅を疑われるので決して"
        "行ってはいけないことについて十分理解した旨述べ、夫に対してもそのようなこと"
        "を行ってはいけないと強く言い聞かせると誓約している（添付・上申書、誓約書）"
        "。"
    ))
    paras.append(mk_p(
        "aff1",
        "また、本件接見は警察署職員の立会いの下で行われ、手紙も検閲を経るもの"
        "であるから、これらの手続的担保を潜脱して罪証隠滅が現実に行われる具体的危険"
        "性については、検察官において何ら主張・疎明がなされていない。一般人との文書"
        "の授受はすべて検閲され、接見も警察署職員などの立ち会いの下に会話の内容も極"
        "めて限定されている以上、通信手段を通じて、あるいは一般人を介して、罪証の隠"
        "滅をはかろうとしても、実際には不可能である。"
    ))
    paras.append(mk_p(
        "a1",
    ...
    paras.append(mk_p(
        "aff1",
        "しかし、別の被疑者に関する判断結果を述べるにとどまるものであり、本件"
        "被疑者と内縁の妻との接見禁止を基礎付ける具体的事情とはなり得ない。"
    ))

    paras.append(mk_p("a0", "　小括"))
    paras.append(mk_p(
        "aff",
        "以上から、勾留に加えて接見等禁止までを必要とする根拠として実質的に機"
        "能し得るのは検察官の挙げた②の点のみであるが、上記のとおりこれも具体性を欠"
        "き、立会接見及び手紙の検閲という手続的担保がある以上、これを潜脱して罪証隠"
        "滅が現実に行われる具体的事情の主張・疎明がない限り、大宮氏との接見及び手紙"
        "の授受を一律に禁止することは到底許されない。"
    ))
    paras.append(mk_empty())
