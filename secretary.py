...
def _nfc(value: str) -> str:
    ...
    macOSのos.listdirは濁点分解（NFD）で返す一方、GAS・シート上のファイル名は
    NFCのため、正規化せずdictキーで突合すると同名でも不一致になり、
    既存行更新のはずが重複行追加になる（260702「書類送付のご案内」で実発生）。
    """
...
def _resolve_client_dest(client, haystack):
    ...
    # キーワードマップの名称（例: 12_精算？）と実フォルダ名（例: 12_精算）の
    # 表記揺れを実在フォルダへ寄せる（無ければ従来どおり作成される）
    if subfolder and not os.path.isdir(os.path.join(client["path"], subfolder)):
        alt = _fuzzy_match_subfolder(client["path"], subfolder)
        if alt:
            subfolder = alt
    if subfolder:
...
- 「プリア常盤台」「プリアときわ台」（社宅）関連の契約・規約・精算・請求は、経理ではなく 04_事務/各種書類/社宅関係。
- 依頼者のための立替払い領収書（謄写代・記録取寄せ等の実費で、但し書き等に依頼者名があるもの）は、03_経理 ではなく `01_事件記録/<依頼者フォルダ名>/12_精算` へ（依頼者への精算対象。事務所経費ではない）。
...
def classify_by_content(
    ...
    # 本文ベースの固定ルール（社宅=プリア常盤台など）。スキャン書類はファイル名・
    # 送信者に手がかりが無く本文にしか「プリア」が現れないため、LLMに投げる前に
    # 外形分類と同じ特別ルールを本文込みで再判定する（260704: 管理規約集・入居時
    # 精算書等が分からなかった／経理未分類へ流れた実例への対処）。
    if has_text:
        special = classify_special_document(
            original_name, f"{sender} {subject} {extracted['text']}"
        )
        if special:
            rel = os.path.relpath(special, os.path.join(BASE_PATH, "共有用"))
            logging.info(f"本文特別ルール適用: {original_name} → {rel}")
            return (
                rename_file(original_name),
                special,
                rel,
                "本文特別ルール: 社宅（プリア常盤台）関連の記載を本文中に検出",
                {
                    "requires_bengokakumei": False,
...
def build_filename_index(sheet_data):
    ...
    for i, row in enumerate(sheet_data):
        ...
        if len(row) >= 4 and row[3]:
            ...
            sender = row[2] if len(row) >= 3 else ""
            dest = row[4] if len(row) >= 5 else ""
            # キーはNFC正規化（macOSのNFDファイル名と突合できるように）
            index[_nfc(str(row[3]))] = {
                "row": i + 1,  # 1-based行番号
                "subject": subject,
                "sender": sender,
                "dest": dest,
...
def _collect_pending_files(sheet_data) -> list[dict]:
    ...
    try:
        actual_files = set(
            _nfc(n) for n in os.listdir(unknown_dir) if not n.startswith(".")
    ...
    for i, row in enumerate(sheet_data):
        ...
        # ゴースト行除外: 実フォルダに無いならスキップ（NFCで突合）
        if _nfc(filename) not in actual_files:
            logging.info(
...
def main():
    ...
    for original_name, src_path in files:
        # 照合キーはNFC（listdirのNFD名とシートのNFC名の不一致による重複行を防ぐ）
        lookup_name = _nfc(original_name)

        # スプレッドシートに既に存在し、E列（移動先）が埋まっている → 処理済みとしてスキップ
        if lookup_name in filename_index and filename_index[lookup_name]["dest"]:
            skipped += 1
            continue
        ...
        meta = filename_index.get(lookup_name, {})
        ...
        # 03_経理/未分類 行き（経理書類だが勘定科目が外形から不明）も LLM で
        # 勘定科目・4フィールド名まで確定を試みる（低信頼なら未分類のまま維持）
        # 汎用ファイル名（receipt, scan 等）は分類成功でも LLM でリネームのみ実施
        l_note = ""
        llm_extras: dict = {}
        if dest_dir == UNKNOWN_FOLDER or dest_description.rstrip("/") == "03_経理/未分類":
        ...
        if os.path.exists(intended_path) and files_identical(src_path, intended_path):
            ...
            existing = (
                filename_index.get(lookup_name) or filename_index.get(_nfc(new_name))
            )
            if existing:
                update_sheet_dest(
                    sheets_service,
        ...
        existing = (
            filename_index.get(lookup_name) or filename_index.get(_nfc(actual_name))
        )
        if existing:
    ...
    if pending_new:
        ...
        for original_name, actual_name, d_cell, dest_description, l_note in pending_new:
            race_hit = (
                fresh_index.get(_nfc(original_name)) or fresh_index.get(_nfc(actual_name))
            )
            if race_hit and race_hit["row"] not in {
                filename_index.get(_nfc(original_name), {}).get("row"),
                filename_index.get(_nfc(actual_name), {}).get("row"),
            }:
