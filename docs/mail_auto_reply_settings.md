...
Googleスプレッドシートの `設定` シート B2〜B6 に手動反映するためのマージ版。
既存の運用ルールは残し、安全ルールだけを追記・整理している。
GASメニュー「AIメール自動化 > 推奨設定を適用」でこの内容をワンクリック反映できる
（`gas/Utils..js` の `RECOMMENDED_SETTINGS_` / `applyRecommendedSettings()` とマスターを一致させること）。

## B2 AIモデル（返信要否判定用・現行維持）
...
A6セルに見出し「生成用AIモデル」、C6セルに説明「返信文生成に使うAIモデル（返信要否判定はB2のモデルを使用）」を設定する。

```text
gemini-2.5-pro
```

生成のみ gemini-2.5-pro を使用し、返信要否判定は引き続き B2 の gemini-2.5-flash を使う（レスポンス速度・コストの兼ね合い）。
gemini-2.5-pro はレスポンスが遅いため、GASの6分実行制限とバッティングする場合は `processUnreadEmails` の1回あたり処理件数（現在20件）を10件程度に下げること。
