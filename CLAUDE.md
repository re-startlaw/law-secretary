...
- プロンプト: `obsidian-vault/50_プロンプト/`
- ツール・サービス開発: `obsidian-vault/60_ツール開発/{サービス名}/`
...
**何を頼まれても、作業完了後に必ず該当フォルダのノートに記録を残すこと。**

| 依頼の種類 | 記録先 |
|-----------|--------|
| 事件・依頼者に関する作業 | `10_事件メモ/{依頼者名}/` |
| 業務手順・ナレッジ・判例 | `20_業務ナレッジ/` |
| ツール・サービス開発（Inbox Curtain等） | `60_ツール開発/{サービス名}/進捗ログ.md` |
| その他（一般的な作業・相談） | `20_業務ナレッジ/` または最も近いフォルダ |
...
既存のノートがあれば追記、なければ新規作成する。
ファイル名の例：`進捗ログ.md`（継続案件）、`{日付}_{トピック}.md`（単発作業）
...
- 常に日本語で応答する
- セキュリティルールを最優先する
- **作業完了後は必ず Obsidian Vault の該当ノートに記録を残す**（上記「メモ取得ルール」参照）
